...
// グローバル変数宣言
LPDIRECT3DTEXTURE9 g_pTextureitemUI[ITEMUITYPE_MAX] = {};	// テクスチャへのポインタ	misaki
LPDIRECT3DVERTEXBUFFER9 g_pVtxBuffitemUI = NULL;			// 頂点バッファへのポインタ	misaki
Item g_aItem[ITEM_MAX] = {};								// オブジェクト情報体
ItemManager g_ItemManager;									// ファイル情報体
ItemUI g_itemUI[ITYPE_MAX];									// アイテムUIの構造体		misaki
...
void InitItem(void)
{
	...
	{// 最大数繰り返す
		...
		g_aItem[nCntItem].Type = ITYPE_1;							// タイプ識別子を初期化する
	...
	{// UIのタイプ分繰り返す
		...
		D3DXCreateTextureFromFile(pDevice,
			ITEM_TEXTURE[nCount],
			&g_pTextureitemUI[nCount]);
	}

	// 頂点バッファの生成
	pDevice->CreateVertexBuffer(sizeof(VERTEX_2D) * 4 * ITYPE_MAX,
		D3DUSAGE_WRITEONLY,
		FVF_VERTEX_2D,
		D3DPOOL_MANAGED,
		&g_pVtxBuffitemUI,
	...
	{// UIの最大数分繰り返す
		g_itemUI[nCount].UIpos = D3DXVECTOR3(1575.0f + (ITEMUI_TEXTUREW_MAX * nCount) + (ITEMUI_DISTANCE_MAX * nCount), 290.0f, 0.0f);		// UIの位置を初期化する
		g_itemUI[nCount].UIcol = D3DXCOLOR(1.0f, 1.0f, 1.0f, 1.0f);																			// UIの色を初期化する
		g_itemUI[nCount].fWidth = ITEMUI_TEXTUREW_MAX;																						// UIの横幅を初期化する
		g_itemUI[nCount].fHeight = ITEMUI_TEXTUREH_MAX;																						// UIの縦幅を初期化する
		g_itemUI[nCount].bUse = false;																										// アイテムを所持しているかを初期化する

		// 頂点座標の設定
		pVtx[0 + (4 * nCount)].pos = D3DXVECTOR3(g_itemUI[nCount].UIpos.x - (g_itemUI[nCount].fWidth * 0.5f), g_itemUI[nCount].UIpos.y - (g_itemUI[nCount].fHeight * 0.5f), 0.0f);
		pVtx[1 + (4 * nCount)].pos = D3DXVECTOR3(g_itemUI[nCount].UIpos.x + (g_itemUI[nCount].fWidth * 0.5f), g_itemUI[nCount].UIpos.y - (g_itemUI[nCount].fHeight * 0.5f), 0.0f);
		pVtx[2 + (4 * nCount)].pos = D3DXVECTOR3(g_itemUI[nCount].UIpos.x - (g_itemUI[nCount].fWidth * 0.5f), g_itemUI[nCount].UIpos.y + (g_itemUI[nCount].fHeight * 0.5f), 0.0f);
		pVtx[3 + (4 * nCount)].pos = D3DXVECTOR3(g_itemUI[nCount].UIpos.x + (g_itemUI[nCount].fWidth * 0.5f), g_itemUI[nCount].UIpos.y + (g_itemUI[nCount].fHeight * 0.5f), 0.0f);
		...
		// テクスチャ座標の設定
		pVtx[0 + (4 * nCount)].tex = D3DXVECTOR2((float)(1.0f / 3 * nCount), 0.0f);
		pVtx[1 + (4 * nCount)].tex = D3DXVECTOR2((float)(1.0f / 3 * (nCount + 1)), 0.0f);
		pVtx[2 + (4 * nCount)].tex = D3DXVECTOR2((float)(1.0f / 3 * nCount), 1.0f);
		pVtx[3 + (4 * nCount)].tex = D3DXVECTOR2((float)(1.0f / 3 * (nCount + 1)), 1.0f);
	}
...
void LoadItemManager(void)
{
	...
	//Xファイル
	pFile = fopen(IMANAGER_FILE, "r");//ファイルを開く
	if (pFile != NULL)
	{//開けたら
