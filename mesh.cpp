...
//2DのYX軸の頂点バッファを生成する
//----------------------------------------------------
HRESULT SetVertex2DYX(LPDIRECT3DVERTEXBUFFER9* ppVtxBuff, int StartIdx, int BuffMax, int Ynum, int Xnum, float Width, float Height, float rhw, D3DXCOLOR col, float TexWidth, float TexHeight)
{
	HRESULT hr; //成功確認
	...
	{//指定数ループ
		...
		{//1枚分のZ軸のループ
			...
			{//1枚分のX軸のループ
				...
				//nor
				pVtx[nCnt2 * (Xnum + 1) + nCnt3].rhw = rhw;
...
//2DのZX軸の頂点バッファを生成する
//----------------------------------------------------
HRESULT SetVertex2DZX(LPDIRECT3DVERTEXBUFFER9* ppVtxBuff, int StartIdx, int BuffMax, int Znum, int Xnum, float Width, float Depth, float rhw, D3DXCOLOR col, float TexWidth, float TexDepth)
{
	HRESULT hr; //成功確認
	VERTEX_2D* pVtx;//頂点情報ポインタ
	int nCnt, nCnt2, nCnt3; //カウント変数
	...
	{//指定数ループ
		...
		{//1枚分のZ軸のループ
			...
			{//1枚分のX軸のループ
				//座標設定
				pVtx[nCnt2 * (Xnum + 1) + nCnt3].pos = D3DXVECTOR3(-Width * 0.5f + (Width / Xnum) * nCnt3, 0.0f, Depth - (Depth / Znum) * nCnt2);
